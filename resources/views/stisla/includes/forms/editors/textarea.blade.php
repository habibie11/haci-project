...
array_push($props, isset($height) ? 'style="height: ' . $height . ';"' : '');
@endphp
...
<div class="form-group">
  ...
  <textarea rows="12" {!! implode(' ', $props) !!} class="form-control {{ $errors->has($name ?? $id) ? ' is-invalid'
    : '' }}">{{ old($name ?? $id) ?? ($value ?? ($d[$name ?? $id] ?? '')) }}</textarea>
  @if ($hint ?? false)
...
<div class="form-group {{ $errors->has($name ?? $id) ? 'has-error' : '' }}">
  ...
  <textarea @if ($placeholder ?? false) placeholder="{{ $placeholder }}" @endif name="{{ $name ?? $id }}" @isset($id)
    id="{{ $id }}" @endisset @isset($disabled) disabled @endisset @if ($required ?? false) required @endif @if ($min ??
    false) min="{{ $min }}" @endif class="form-control">{{ old($name ?? $id, $d->$name ?? ($d->$id ?? '')) }}</textarea>
